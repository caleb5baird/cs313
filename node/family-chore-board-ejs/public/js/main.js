...
$(document).ready(function() {
	getUser((err, users)=>{
		let user = users[0];
		$('#accountBalance').html(user.accountbalance);
		$('#streakNumber').html(user.streak);
		$('.name').html(user.name);
		loadToDo(user.id)
		loadDone(user.id)
		loadAdditionalChores(user.id)
	});
})

function loadToDo(userId){
	loadChoreCategory(userId, 'to-do');
}

function loadDone(userId){
	loadChoreCategory(userId, 'done');
}

function loadAdditionalChores(userId){
	loadChoreCategory(userId, 'unassigned');
}

function loadChoreCategory(userId, category){
}
